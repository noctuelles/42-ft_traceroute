...
/*   Updated: 2024/06/26 12:00:30 by plouvel          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
traceroute(const t_fd_sock *res_host, t_fd_sock *peer) {
    ...
    timeout.tv_usec = 0;
    printf("traceroute to %s (%s), %u hops max\n", res_host->name, res_host->ipv4, g_opts.max_hops);
    while (true) {
        if (send_probe(res_host->fd, 1, (const struct sockaddr *)&res_host->addr, sizeof(res_host->addr)) == -1) {
            return (-1);
        }
        if ((ret = select(peer->fd + 1, &readfds, NULL, NULL, &timeout)) == -1) {
            ft_error(0, 0, "select: %s", strerror(errno));
            return (-1);
        } else if (ret == 0) {
            puts("Timeout");
        } else {
            if (recv_packet(peer->fd, &res_host->addr.sin_addr) != 0) {
                ft_error(1, 0, "error happened\n");
            }
        }
        FD_SET(peer->fd, &readfds);
        timeout.tv_sec  = g_opts.wait_time != 0 ? g_opts.wait_time : DFT_WAIT_TIME_SEC;
